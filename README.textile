...
4. Create a config/dependencies.rb file that looks something like:

    dependency 'gem'
    dependency 'gem', '1.0.1' # use a specific version
    dependency 'gem', :require_as => 'Gem' # specify what to pass the "require" keyword (use nil to avoid require!)
